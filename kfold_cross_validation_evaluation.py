...
def evaluate_kfold_naive_bayes(df, df1):
    ...
    for repeat in range(num_repeats):
        ...
        for train_index, test_index in kf.split(raw_shuffled_indices):
            # Indices escolhidos para serem avaliados
            ...
            # Localiza as colunas que definem se é spam ou não
            ...
            # Transforma o resultado em vetores
            ...
            start_time = time.time() # Começa avaliação de tempo de execução do algoritmo
            raw_model = MultinomialNB().fit(X_raw_train_vectorized, y_kfold_train)
            raw_predictions = raw_model.predict(X_raw_test_vectorized)
            raw_accuracy = accuracy_score(y_kfold_test, raw_predictions) # Acuracia
            raw_f1 = f1_score(y_kfold_test, raw_predictions) # F1-Score
            end_time = time.time() # Finaliza avaliação de tempo de execução do algoritmo
            # Concatena os resultados para poderem ser visualizados
            raw_fold_execution_times.append(end_time - start_time)
            raw_fold_results.append(raw_accuracy)
            raw_fold_f1_scores.append(raw_f1)
            
            # Processo é repetido para o dataset pre processado
    ...
    # Tira a média dos resultados e retorna para serem plotados
